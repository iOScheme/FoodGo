...
public protocol RequestProviderProtocol {
    func execute<T: Decodable>(endpoint: NetworkLayerEndpoint, dto: T.Type)  async throws -> T
}
...
public final class RequestProvider: RequestProviderProtocol {
    ...
    public func execute<T: Decodable>(endpoint: NetworkLayerEndpoint, dto: T.Type)  async throws -> T {
        guard let request = endpoint.asURLRequest else { throw URLError(.badURL)}
        return try await networkLayer.execute(request: request, dto: dto)
    }
}
